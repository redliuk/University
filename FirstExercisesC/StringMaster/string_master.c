...
/*** TO BE COMPLETED START ***/
	...
	for (j=2;j<sizeof(c) ; j++)
		printf("%c",c[j]);
	printf("\nAnche \"p\" e` un puntatore a carattere esattamente come \"q\",\nma poiche` e` stato"
		"inizializzato al valore NULL (che corrisponde al valore 0),\nnon puo` essere usato "
		...
		"eseguendo l'istruzione \"p=c;\" lo si puo` correttamente usare per indirizzare\n"
		"la memoria, per esempio eseguendo l'istruzione\n"
		"if(argc>1&&strlen(argv[1])>1&&argv[1][1]==p[0])i+=2;\n");

	if ( argc > 1 && strlen(argv[1]) && i!=1)  {
		if ( *(argv[1]) == 'm' && *(argv[1]+1) == 'p' )
			i = 2;
	}
