...
/*** TO BE DONE 4.0 START ***/
	...
	struct tm* date_serv=gmtime(&now_t);
	now_tm=*date_serv;
...
/*** TO BE DONE 4.0 START ***/
        	...
        	sprintf(http_header + strlen(http_header), "\r\nSet-Cookie: UserID=%d", cookie);
        	strcat(http_header, "; Expires= Mon, 31 Dec 2021 23:59:59 GMT");
...
/*** TO BE DONE 4.0 START ***/
		...
		struct tm *srv_time=gmtime(&file_modification_time);
		file_modification_tm=*srv_time;
		if(strftime(time_as_string, MAX_TIME_STR, "%a, %d %b %Y %T GMT", &file_modification_tm)<0)
			perror("Failed making string from time");
...
/*** TO BE DONE 4.0 START ***/
		...
		if (sendfile(client_fd, fd, NULL, file_size) < 0) perror("Generated warning by sendfile()");
		close(fd);
...
/*** TO BE DONE 4.0 START ***/
			    	...
			    	if(strcmp(strtok_r(NULL, "=", &strtokr_save), " UserID")==0)
			    	{
			    		option_val=strtok_r(NULL, "\r\n", &strtokr_save);
			    		if(option_val!=NULL )	UIDcookie=atoi(option_val);
			    		if(UIDcookie==0) UIDcookie=-1;
			    	}
			    	else
				    	SEND_RESPONSE(client_fd, RESPONSE_CODE_BAD_REQUEST, UIDcookie, NULL, NULL);	
...
/*** TO BE DONE 4.0 START ***/
			    	...
			    	if(option_name!=NULL)
			    	{
			    		...
			    		{
			    			char* timeing=strtok_r(NULL, "\r\n", &strtokr_save);
							if(timeing!=NULL && strptime(timeing, "%a, %d %b %Y %T GMT", &since_tm)!=NULL)
			    			{
...
/*** TO BE DONE 4.0 START ***/
				...
				if(difftime(my_timegm(&since_tm), stat_p->st_mtime))
